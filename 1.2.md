...
todos:
  ...
  - id: srv2-12
    ...
    dependencies:
      - srv2-6
  - id: srv2-13
    content: Usar pthread_sigmask(SIG_BLOCK) nas threads gestoras para bloquear SIGUSR1
    status: pending
    dependencies:
      - srv2-7
  - id: srv2-14
...
### 14. Usar pthread_sigmask nas threads gestoras

- **Arquivo**: `src/server/game.c` (função `session_manager_thread_func()`)
- No início da função, bloquear `SIGUSR1`:
- `sigset_t set; sigemptyset(&set); sigaddset(&set, SIGUSR1);`
- `pthread_sigmask(SIG_BLOCK, &set, NULL);`
- Apenas thread anfitriã recebe `SIGUSR1` (para Exercício 2)

### 15. Garantir thread safety no buffer
...
### 16. Limpar recursos do buffer na terminação
...
### 17. Aguardar todas as threads gestoras terminarem
